...
/* ---------- UI ---------- */
function shell(title,body,script=""){return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#111827"><title>${title}</title><style>
*{box-sizing:border-box}html{font-size:16px}body{margin:0;background:#0b1220;color:#e5e7eb;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;min-height:100dvh;padding:max(12px,env(safe-area-inset-top)) max(12px,env(safe-area-inset-right)) max(20px,env(safe-area-inset-bottom)) max(12px,env(safe-area-inset-left))}
a{color:#93c5fd}.wrap{max-width:1100px;margin:auto}.top{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;margin:8px 0 14px}.top h1{font-size:20px;margin:0}.muted{color:#94a3b8;font-size:13px}.card{background:#111827;border:1px solid #273449;border-radius:14px;padding:14px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.controls{display:flex;gap:8px;flex-wrap:wrap}.input,select{background:#0f172a;color:#e5e7eb;border:1px solid #334155;border-radius:10px;padding:11px 12px;min-height:42px;outline:none}.input:focus,select:focus{border-color:#60a5fa}.btn{border:0;border-radius:10px;padding:10px 13px;min-height:42px;background:#2563eb;color:#fff;font-weight:650;cursor:pointer}.btn.secondary{background:#334155}.btn.danger{background:#b91c1c}.btn.small{min-height:36px;padding:8px 10px;font-size:13px}.table{width:100%;border-collapse:collapse}.table th,.table td{text-align:left;padding:10px 8px;border-bottom:1px solid #223047;vertical-align:top;font-size:14px}.table th{color:#94a3b8;font-weight:650}.msg{white-space:pre-wrap;overflow-wrap:anywhere;line-height:1.45}.pill{display:inline-block;padding:3px 8px;border-radius:999px;background:#1e293b;color:#cbd5e1;font-size:12px}.nav{display:flex;gap:8px;flex-wrap:wrap}.toast{position:fixed;right:12px;bottom:12px;background:#1e293b;border:1px solid #334155;padding:11px 13px;border-radius:10px;z-index:20;max-width:90vw}.hide{display:none}.row{display:flex;justify-content:space-between;gap:10px;align-items:center}.danger-text{color:#fca5a5}
@media(max-width:720px){.grid{grid-template-columns:1fr}.top{align-items:stretch}.top .nav{width:100%}.top .nav .btn{flex:1}.table thead{display:none}.table,.table tbody,.table tr,.table td{display:block;width:100%}.table tr{border:1px solid #273449;border-radius:12px;margin:8px 0;background:#0f172a}.table td{border:0;border-bottom:1px solid #1e293b;display:flex;justify-content:space-between;gap:12px;text-align:right;overflow-wrap:anywhere}.table td:last-child{border-bottom:0}.table td:before{content:attr(data-label);color:#94a3b8;font-weight:650;text-align:left;flex:none}.controls>*{width:100%}.btn{width:100%}.inline{display:flex!important;gap:8px}.inline>*{width:auto!important;flex:1}.msg{text-align:left}.table td[data-label="操作"]{display:block;text-align:left}.table td[data-label="操作"]:before{display:block;margin-bottom:7px}}
...
function dashboardPage(s){
  ...
  <div class="card"><div class="row"><b>消息</b><span id="total" class="muted"></span></div><div style="overflow:auto"><table class="table"><thead><tr><th>消息</th><th>已读</th><th>时间</th><th>操作</th></tr></thead><tbody id="rows"></tbody></table></div></div>`;
...
function readPage(s,m){
  ...
  <div class="card"><div class="row"><b>访问记录</b><span id="stat" class="muted"></span></div><div style="overflow:auto"><table class="table"><thead><tr><th>IP</th><th>UA</th><th>时间</th><th>地区</th><th>操作</th></tr></thead><tbody id="reads"></tbody></table></div></div>`;
...
async function block(ip){ip=decodeURIComponent(ip);const r=await fetch('/reads/${m.id}/block',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ip})});alert((await r.json()).error||'已拉黑');load()}
async function blockCurrent(){const r=await fetch('/reads/${m.id}/block',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'current'})});alert((await r.json()).error||'已拉黑');load()}
...
function adminPage(){
  ...
  <div class="card"><h3>等级权益公式</h3><div class="controls"><input id="fm" class="input" value="x" placeholder="message"><input id="fg" class="input" value="x" placeholder="geo"><input id="fr" class="input" value="x" placeholder="retention"><button class="btn" onclick="saveFormula()">保存</button></div><p class="muted">支持 x、+ - * / % ^、括号及 floor/ceil/round/abs/min/max/pow。</p></div>
  <div class="card"><h3>用户</h3><div class="controls"><input id="uq" class="input" placeholder="搜索 wxId"><button class="btn" onclick="users()">刷新</button></div><div style="overflow:auto"><table class="table"><thead><tr><th>wxId</th><th>等级</th><th>消息数</th><th>累计注册</th><th>操作</th></tr></thead><tbody id="us"></tbody></table></div></div>
  ...
  <div class="card"><h3>僵尸用户清理</h3><div class="controls"><input id="nd" class="input" type="number" value="0" placeholder="从未注册天数"><input id="dd" class="input" type="number" value="0" placeholder="沉寂天数"><button class="btn" onclick="retSave()">保存策略</button><button class="btn danger" onclick="retRun()">立即清理</button></div></div>`;
